(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendToWebhook(endpoint, payload, submitBtnEl) {
      ...
      try {
        // 1) Envia ao seu webhook de leads (fluxo atual)
        ...
        // 2) Sucesso: dispara Pixel + CAPI (dedupe por event_id)
        try {
          const email = payload.email || payload.email_comercial || null;
          const phone = payload.telefone || null;
          if (window.metaLeadTrack) {
            await window.metaLeadTrack({ email, phone }); // fbq('trackCustom',..., {eventID}) + POST /api/meta/lead
          }
        } catch (e) {
          console.warn('Falha ao registrar evento Meta (não bloqueia o lead):', e);
        }

        // 3) Redireciona para "enviado"
